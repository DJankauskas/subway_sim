...
impl Simulator {
    pub fn new(subway_map: SubwayMap, routes: Vec<Route>) -> Self {
        ...
        let mut queue: VecDeque<TrackStationId> = terminal_nodes.into_iter().map(|n| TrackStationId::Station(n)).collect();
        ...
        while let Some(track_station) = queue.pop_front() {
            ...
            match track_station {
                ...
                TrackStationId::Station(station) => {
                    subway_map.edges_directed(station, Direction::Incoming).for_each(|track| queue.push_back(TrackStationId::Track(track.id())))
                }
            }
        }
        
        let routes = routes.into_iter().enumerate().map(|(i, route)| (RouteId(i as u32), route)).collect();
    ...
    pub fn run(&mut self, mut t: u32) -> Vec<TrainPositions> {
        ...
        while t > 0 {
                ...
                match *track_station {
                    ...
                    TrackStationId::Track(track) => {
                        ...
                        while i < track_mut.trains.len() {
                            ...
                            let mut time_left = TIME_STEP;
                            let travel_distance = f64_min(time_left, f64_max(last_train_pos - curr_train_mut.pos - MIN_TRAIN_DISTANCE, 0.0));
                            ...
                            if curr_train_mut.pos >= track_mut.length as f64 {
                                ...
                                debug_assert!(self.stations.get_mut(&next_station_id).unwrap().train.is_none());
                                self.stations.get_mut(&next_station_id).unwrap().train = Some(curr_train_id);
                                curr_train_mut.pos = time_left;
                                curr_train_mut.curr_section = TrackStationId::Station(next_station_id);
            ...
            for (id, train) in &self.trains {
                curr_train_positions.push(TrainPosition { id: *id, curr_section: train.curr_section, pos: train.pos })
            }
            train_positions.push(TrainPositions { time: t, trains: curr_train_positions });
